...
int main(int argc, char**) {
    ...
    try {
        ...
        options.petsc_ordering = "natural"; // 通量先消元，再消元零质量压力块和均值乘子。
        options.ilu_levels = 0;
        options.equation = "darcy";
        ...
        options.error_interval = 0;    // 0 仅最终步做误差积分。
        options.print_interval = 10;   // 首步、每 10 步及最终步打印进度和耗时。

        // ========== 输出目录与累计表名称 ==========
        const std::string output_root = "output/Darcy_curved_k" + std::to_string(k);
        ...
        study.title = "Darcy 算例 1：正弦曲边压力/通量空间收敛实验";
        ...
        std::cerr << "Darcy_curved 运行失败：" << e.what() << '\n';
        return 1;
    }
}
