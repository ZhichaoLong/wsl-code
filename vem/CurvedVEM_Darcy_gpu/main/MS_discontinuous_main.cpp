/**
 * MS 算例 1：间断初值扩散与组分质量守恒。
 * 此算例无解析解：终端输出每组分质量、初始质量及漂移，不伪造误差和收敛阶。
 * 默认只计算一套网格的长时间过程，定期输出进度/质量，最终输出耗时。
 * 编译：make MS_discontinuous；运行：./build/MS_discontinuous。
...
int main(int argc, char**) {
    ...
    if (argc != 1) {
        std::cerr << "请在 main/MS_discontinuous_main.cpp 中修改参数。\n";
        return 1;
    }
    try {
        // ========== 算例、空间阶与积分规则 ==========
        ...
        // ========== 输出目录与累计表名称 ==========
        const std::string output_root = "output/MS_discontinuous_k" + std::to_string(k);
        ...
        study.title = "MS 算例 1：间断初值扩散与组分质量守恒";
        ...
        std::cerr << "MS_discontinuous 运行失败：" << e.what() << '\n';
        return 1;
    }
}
