...
int main(int argc, char**) {
    ...
    if (argc != 1) {
        std::cerr << "请在 main/Darcy_straight_main.cpp 中修改参数。\n";
        return 1;
    }
    try {
        // ========== 算例、空间阶与积分规则 ==========
        ...
        // ========== 输出目录与累计表名称 ==========
        const std::string output_root = "output/Darcy_straight_k" + std::to_string(k);
        ...
        study.title = "Darcy 算例 2：恒等映射压力/通量空间收敛实验";
        ...
        std::cerr << "Darcy_straight 运行失败：" << e.what() << '\n';
        return 1;
    }
}
