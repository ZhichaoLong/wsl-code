...
SolveInfo Krylov::solve(MixedGpu& equation, const PetscVector& b, PetscVector& x, double tol, int maxit, int restart) {
    ...
    if (changed) {
        ...
        if (assembled_) {
            ...
            if (pc_ == "ilu" || pc_ == "lu") {
                PETSC(PCFactorSetMatSolverType(pc, MATSOLVERCUSPARSE));
                if (pc_ == "ilu")
                    PETSC(PCFactorSetLevels(pc, levels_));
                // 此 PETSc 版本的 GPU ILU(0) 忽略 shift；仅 CPU 数值分解支持该保护。
                if (pc_ == "lu" || levels_ > 0)
    ...
    if (assembled_) {
        ...
        if (pc_ == "ilu" || pc_ == "lu") {
            ...
            out << " factor_backend=" << backend << " global_ordering=" << ordering_
                ...
                << " numeric_factorization=" << ((pc_ == "ilu" && levels_ == 0) ? "GPU" : "CPU")
                << " triangular_solve=GPU";
        }
    }
    out << '\n' << std::flush;
}
